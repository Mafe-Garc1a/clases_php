...
<div class="modal fade" id="exampleModal9" tabindex="-1" aria-labelledby="exampleModalLabel9" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel9">ingresa usuario</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body text-center">
        ...
        <?php  
        $consultaR=$conexion->query("SELECT * FROM roles");
        while($rowR=$consultaR->fetch_array()){
            echo '<option value="'.$rowR['id_rol'].'"selected>'.$rowR['nombre_rol'].'</option>';
        };
        ?>
        </select>
        <h2>ingrese la contraseña</h2>
        <input type="password" name="pass_hash">
        <div class="row mt-2">
          <div class="col">
            <button type="submit" class="btn btn-primary" value="enviar"> enviar</button>
          </div>
